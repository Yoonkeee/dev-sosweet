...
const ModifyDog = ({ isOpen, onClose }) => {
  ...
  const { isLoading, data } = useQuery({ queryKey: ['dog_info', name], queryFn: getDogInfo });
  const options =
    selectData &&
    selectData?.map(item => ({
      value: item.name,
  ...
  return (
    <Modal isCentered isOpen={isOpen} onClose={onCloseFn}>
      <ModalOverlay />
      <ModalContent>
        <ModalHeader>🐶{data && data.length !== 0 ? name : '댕댕이'}🥰 정보</ModalHeader>
        <ModalCloseButton />
        <ModalBody as="form" onSubmit={handleSubmit(onSubmit)}>
          <VStack w="100%">
            ...
            <HStack w="100%">
              ...
              <Input minW="55%" mr={1} placeholder="견종(선택)" variant="filled" {...register('dogBreed')} />
